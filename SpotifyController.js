 ...
 */
 ...
 // TODO: why do controls render if the host is unreachable?
...
function renderControlsIfHostIsReachable() {
	if(currentUser === account || currentUser == null) {
		getState(renderControls);
	}
}

function renderControls(state) {

	if(currentUser != null && currentUser != account)
		return;
...
/*
 * Local endpoint methods
 *
 */
function getState(callback) {
	var url = host + "/getState";
	get(url, callback);
}
...
function togglePlay() {
	var url = host + "/toggle";
	get(url, updatePlayPauseControl);
}
...
function updatePlayPauseControl(playState) {
	$('div#spotify-controls span#toggle-play').html(iconFactory(playState.status == "playing" ? "pause" : "play"));
}

function setVolumeFromSlider() {
	setVolume($('div#spotify-controls input#volume').val());
}


/*
 * Public Spotify endpoint methods
 *
 */
